...
| **Phần 8** | Hệ thống đăng nhập Supabase Auth + multi-tenant | ✅ Hoàn thành |
...
**Hệ thống đăng nhập (Phần 8)**: Supabase Auth — mỗi nhà hàng có 1 tài khoản riêng. `owner_id UUID REFERENCES auth.users(id)` thêm vào bảng `restaurants` (migration 007). `AuthContext` (`dashboard/src/contexts/AuthContext.jsx`) cung cấp `{ user, restaurant, restaurantId, loading, signOut, reloadRestaurant }`. `ProtectedPage` component redirect về `/login` nếu chưa đăng nhập (spinner khi đang load). Các trang bảo vệ: `/` (dashboard), `/nhan-vien`, `/quan-ly-ban`, `/settings`. Trang `/dat-ban` vẫn public, nhận `?r=restaurant_id` từ URL thay vì hardcode. Trang `/login` dùng `supabase.auth.signInWithPassword()`. Trang `/register` tạo tài khoản → gọi Edge Function `restaurant-setup`. `/settings` có section "Tài Khoản" hiển thị tên nhà hàng + email + nút "Đăng xuất" với confirm dialog. Row Level Security (RLS) bật trên tất cả 5 bảng. `restaurantId` null guard ở mọi hook/effect để tránh fetch trước khi auth load xong.

**Bug fix AuthContext — loading spinner vĩnh viễn**: `setLoading(false)` phải được gọi NGAY SAU KHI biết auth state, không phụ thuộc vào `loadRestaurant` hoàn thành. Bug cũ: cả `initAuth()` và `onAuthStateChange` callback đều `await loadRestaurant(u.id)` trước khi gọi `setLoading(false)` — nếu query Supabase hang (mạng chậm, token refresh chưa xong) thì `loading` mãi là `true`. Fix: `loadRestaurant` chạy fire-and-forget (không await), `setLoading(false)` chạy ngay sau `setUser(u)`. `onAuthStateChange` callback không còn `async`. `initAuth()` bọc trong try-catch để failsafe: luôn gọi `setLoading(false)` dù có lỗi. Hiện tượng: xảy ra chủ yếu lần 2 vì access token hết hạn → Supabase phải refresh → thêm latency → cả 2 code path đều bị block.

**Email verification callback**: Route `/auth/callback` (`dashboard/src/app/auth/callback/page.jsx`) xử lý redirect sau khi user click link xác nhận email. Hỗ trợ cả 2 flow của Supabase JS v2: PKCE (`?code=xxx` → `exchangeCodeForSession`) và token-hash (`?token_hash=xxx&type=signup` → `verifyOtp`). `signUp()` trong `/register` truyền `options.emailRedirectTo: window.location.origin + '/auth/callback'` để Supabase biết redirect về đâu. Cần cấu hình trong Supabase Dashboard → Authentication → URL Configuration: Site URL = `http://localhost:3001`, Redirect URLs thêm `http://localhost:3001/**`. `AuthContext` cũng có safety net: xử lý `?code=` param ở bất kỳ trang nào (phòng khi Site URL config sai khiến Supabase redirect về homepage thay vì `/auth/callback`).

**Section "Link Đặt Bàn" trong /settings**: Nằm giữa section Giao Diện và Cấu Hình Đặt Bàn. Hiện URL `[origin]/dat-ban?r=[restaurantId]` (computed từ `window.location.origin`). Nút "Copy link" dùng `navigator.clipboard.writeText()` với fallback `execCommand('copy')` cho trình duyệt cũ; hiện "Đã copy!" trong 2 giây (state `copied`). Nút "Mở thử" là `<a target="_blank">` mở trang đặt bàn trong tab mới.
