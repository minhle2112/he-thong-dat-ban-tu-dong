...
**Hệ thống đăng nhập (Phần 8)**: Supabase Auth — mỗi nhà hàng có 1 tài khoản riêng. `owner_id UUID REFERENCES auth.users(id)` thêm vào bảng `restaurants` (migration 007). `AuthContext` (`dashboard/src/contexts/AuthContext.jsx`) cung cấp `{ user, restaurant, restaurantId, loading, signOut, reloadRestaurant }` — load từ `GET /api/restaurants/mine?owner_id=uuid` sau khi Supabase session resolve. `ProtectedPage` component redirect về `/login` nếu chưa đăng nhập (spinner khi đang load). Các trang bảo vệ: `/` (dashboard), `/nhan-vien`, `/quan-ly-ban`, `/settings`. Trang `/dat-ban` vẫn public, nhận `?r=restaurant_id` từ URL thay vì hardcode. Trang `/login` dùng `supabase.auth.signInWithPassword()`. Trang `/register` tạo tài khoản → gọi `POST /api/restaurants/setup` (idempotent: claim nhà hàng 1 nếu chưa có owner, hoặc tạo mới với seed 10 bàn). `/settings` có section "Tài Khoản" hiển thị tên nhà hàng + email + nút "Đăng xuất" với confirm dialog. Row Level Security (RLS) bật trên tất cả 5 bảng — bảo vệ khi client truy cập Supabase trực tiếp; backend dùng service role nên bypass RLS. Backend không verify JWT — trust `owner_id` từ frontend (acceptable cho internal tool). `restaurantId` null guard ở mọi hook/effect để tránh fetch trước khi auth load xong.
